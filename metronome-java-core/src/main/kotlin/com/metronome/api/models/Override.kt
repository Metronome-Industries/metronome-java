...
class Override @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("override_tiers") @ExcludeMissing overrideTiers: JsonField<List<OverrideTier>> = JsonMissing.of(),
    ...
    class Builder internal constructor() {
        ...
        fun overrideTiers(overrideTiers: List<OverrideTier>) = overrideTiers(JsonField.of(overrideTiers))

        /**
         * Sets [Builder.overrideTiers] to an arbitrary JSON value.
         *
         * You should usually call [Builder.overrideTiers] with a well-typed `List<OverrideTier>` value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun overrideTiers(overrideTiers: JsonField<List<OverrideTier>>) =
            apply {
         ...
         * @throws IllegalStateException if the field was previously set to a non-list.
         */
        fun addOverrideTier(overrideTier: OverrideTier) =
            apply {
                overrideTiers = (overrideTiers ?: JsonField.of(mutableListOf())).also {
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (if (startingAt.asKnown().isPresent) 1 else 0) + (applicableProductTags.asKnown().getOrNull()?.size ?: 0) + (creditType.asKnown().getOrNull()?.validity() ?: 0) + (if (endingBefore.asKnown().isPresent) 1 else 0) + (if (entitled.asKnown().isPresent) 1 else 0) + (if (isCommitSpecific.asKnown().isPresent) 1 else 0) + (if (isProrated.asKnown().isPresent) 1 else 0) + (if (multiplier.asKnown().isPresent) 1 else 0) + (overrideSpecifiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (overrideTiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (overwriteRate.asKnown().getOrNull()?.validity() ?: 0) + (if (price.asKnown().isPresent) 1 else 0) + (if (priority.asKnown().isPresent) 1 else 0) + (product.asKnown().getOrNull()?.validity() ?: 0) + (if (quantity.asKnown().isPresent) 1 else 0) + (rateType.asKnown().getOrNull()?.validity() ?: 0) + (target.asKnown().getOrNull()?.validity() ?: 0) + (tiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (value.asKnown().getOrNull()?.validity() ?: 0)
    ...
    /** Only set for CUSTOM rate_type. This field is interpreted by custom rate processors. */
    class Value @JsonCreator private constructor(
        @com.fasterxml.jackson.annotation.JsonValue private val additionalProperties: Map<String, JsonValue>,
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Override && id == other.id && createdAt == other.createdAt && startingAt == other.startingAt && applicableProductTags == other.applicableProductTags && creditType == other.creditType && endingBefore == other.endingBefore && entitled == other.entitled && isCommitSpecific == other.isCommitSpecific && isProrated == other.isProrated && multiplier == other.multiplier && overrideSpecifiers == other.overrideSpecifiers && overrideTiers == other.overrideTiers && overwriteRate == other.overwriteRate && price == other.price && priority == other.priority && product == other.product && quantity == other.quantity && rateType == other.rateType && target == other.target && tiers == other.tiers && type == other.type && value == other.value && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(id, createdAt, startingAt, applicableProductTags, creditType, endingBefore, entitled, isCommitSpecific, isProrated, multiplier, overrideSpecifiers, overrideTiers, overwriteRate, price, priority, product, quantity, rateType, target, tiers, type, value, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "Override{id=$id, createdAt=$createdAt, startingAt=$startingAt, applicableProductTags=$applicableProductTags, creditType=$creditType, endingBefore=$endingBefore, entitled=$entitled, isCommitSpecific=$isCommitSpecific, isProrated=$isProrated, multiplier=$multiplier, overrideSpecifiers=$overrideSpecifiers, overrideTiers=$overrideTiers, overwriteRate=$overwriteRate, price=$price, priority=$priority, product=$product, quantity=$quantity, rateType=$rateType, target=$target, tiers=$tiers, type=$type, value=$value, additionalProperties=$additionalProperties}"
}
