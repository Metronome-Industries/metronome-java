 ...
 *
 */
class AlertCreateParams private constructor(
     ...
     *
     * Unlike [billableMetricId], this method doesn't throw if the JSON field has an unexpected type.
     */
     ...
     *
     * Unlike [evaluateOnCreate], this method doesn't throw if the JSON field has an unexpected type.
     */
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(alertCreateParams: AlertCreateParams) =
            apply {
         ...
         *
         * You should usually call [Builder.alertType] with a well-typed [AlertType] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun alertType(alertType: JsonField<AlertType>) =
            apply {
                body.alertType(alertType)
            }
         ...
         *
         * You should usually call [Builder.billableMetricId] with a well-typed [String] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
         ...
         *
         * You should usually call [Builder.evaluateOnCreate] with a well-typed [Boolean] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun evaluateOnCreate(evaluateOnCreate: JsonField<Boolean>) =
            apply {
                body.evaluateOnCreate(evaluateOnCreate)
            }

        /** Only present for `spend_threshold_reached` notifications. Scope notification to a specific group key on individual line items. */
        fun groupValues(groupValues: List<GroupValue>) =
            apply {
        ...
        /** Required for `low_remaining_seat_balance_reached` notifications. The alert is scoped to this seat group key-value pair. */
        fun seatFilter(seatFilter: SeatFilter) =
            apply {
                body.seatFilter(seatFilter)
            }
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
        ...
        @JsonCreator
        private constructor(
            @JsonProperty("alert_type") @ExcludeMissing alertType: JsonField<AlertType> = JsonMissing.of(),
            ...
            @JsonProperty("billable_metric_id") @ExcludeMissing billableMetricId: JsonField<String> = JsonMissing.of(),
            ...
            @JsonProperty("evaluate_on_create") @ExcludeMissing evaluateOnCreate: JsonField<Boolean> = JsonMissing.of(),
         ...
         * @throws MetronomeInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun evaluateOnCreate(): Optional<Boolean> = evaluateOnCreate.getOptional("evaluate_on_create")
        ...
        class Builder internal constructor() {
            ...
            /** If true, the threshold notification will evaluate immediately on customers that already meet the notification threshold. If false, it will only evaluate on future customers that trigger the threshold. Defaults to true. */
            fun evaluateOnCreate(evaluateOnCreate: Boolean) = evaluateOnCreate(JsonField.of(evaluateOnCreate))
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (alertType.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (threshold.asKnown().isPresent) 1 else 0) + (alertSpecifiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (billableMetricId.asKnown().isPresent) 1 else 0) + (creditGrantTypeFilters.asKnown().getOrNull()?.size ?: 0) + (if (creditTypeId.asKnown().isPresent) 1 else 0) + (customFieldFilters.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (customerId.asKnown().isPresent) 1 else 0) + (if (evaluateOnCreate.asKnown().isPresent) 1 else 0) + (groupValues.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (invoiceTypesFilter.asKnown().getOrNull()?.size ?: 0) + (if (planId.asKnown().isPresent) 1 else 0) + (seatFilter.asKnown().getOrNull()?.validity() ?: 0) + (if (uniquenessKey.asKnown().isPresent) 1 else 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Body && alertType == other.alertType && name == other.name && threshold == other.threshold && alertSpecifiers == other.alertSpecifiers && billableMetricId == other.billableMetricId && creditGrantTypeFilters == other.creditGrantTypeFilters && creditTypeId == other.creditTypeId && customFieldFilters == other.customFieldFilters && customerId == other.customerId && evaluateOnCreate == other.evaluateOnCreate && groupValues == other.groupValues && invoiceTypesFilter == other.invoiceTypesFilter && planId == other.planId && seatFilter == other.seatFilter && uniquenessKey == other.uniquenessKey && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(alertType, name, threshold, alertSpecifiers, billableMetricId, creditGrantTypeFilters, creditTypeId, customFieldFilters, customerId, evaluateOnCreate, groupValues, invoiceTypesFilter, planId, seatFilter, uniquenessKey, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Body{alertType=$alertType, name=$name, threshold=$threshold, alertSpecifiers=$alertSpecifiers, billableMetricId=$billableMetricId, creditGrantTypeFilters=$creditGrantTypeFilters, creditTypeId=$creditTypeId, customFieldFilters=$customFieldFilters, customerId=$customerId, evaluateOnCreate=$evaluateOnCreate, groupValues=$groupValues, invoiceTypesFilter=$invoiceTypesFilter, planId=$planId, seatFilter=$seatFilter, uniquenessKey=$uniquenessKey, additionalProperties=$additionalProperties}"
    }

    /** Type of the threshold notification */
    class AlertType @JsonCreator private constructor(
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is AlertCreateParams && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams
    }

    override fun hashCode(): Int = Objects.hash(body, additionalHeaders, additionalQueryParams)

    override fun toString() = "AlertCreateParams{body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
