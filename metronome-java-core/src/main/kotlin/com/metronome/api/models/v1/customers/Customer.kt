...
class Customer @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (externalId.asKnown().isPresent) 1 else 0) + (ingestAliases.asKnown().getOrNull()?.size ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (customFields.asKnown().getOrNull()?.validity() ?: 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Customer && id == other.id && externalId == other.externalId && ingestAliases == other.ingestAliases && name == other.name && customFields == other.customFields && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "Customer{id=$id, externalId=$externalId, ingestAliases=$ingestAliases, name=$name, customFields=$customFields, additionalProperties=$additionalProperties}"
}
