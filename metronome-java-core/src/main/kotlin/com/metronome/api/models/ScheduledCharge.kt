...
class ScheduledCharge @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("schedule") @ExcludeMissing schedule: JsonField<SchedulePointInTime> = JsonMissing.of(),
    ...
    class Builder internal constructor() {
         ...
         *
         * You should usually call [Builder.schedule] with a well-typed [SchedulePointInTime] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun schedule(schedule: JsonField<SchedulePointInTime>) =
            apply {
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (product.asKnown().getOrNull()?.validity() ?: 0) + (schedule.asKnown().getOrNull()?.validity() ?: 0) + (if (archivedAt.asKnown().isPresent) 1 else 0) + (customFields.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (netsuiteSalesOrderId.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is ScheduledCharge && id == other.id && product == other.product && schedule == other.schedule && archivedAt == other.archivedAt && customFields == other.customFields && name == other.name && netsuiteSalesOrderId == other.netsuiteSalesOrderId && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "ScheduledCharge{id=$id, product=$product, schedule=$schedule, archivedAt=$archivedAt, customFields=$customFields, name=$name, netsuiteSalesOrderId=$netsuiteSalesOrderId, additionalProperties=$additionalProperties}"
}
