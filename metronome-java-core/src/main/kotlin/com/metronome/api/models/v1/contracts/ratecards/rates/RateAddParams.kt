 ...
 *
 */
class RateAddParams private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [RateAddParams].
         *
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(rateAddParams: RateAddParams) =
            apply {
                body = rateAddParams.body.toBuilder()
                additionalHeaders = rateAddParams.additionalHeaders.toBuilder()
                additionalQueryParams = rateAddParams.additionalQueryParams.toBuilder()
            }
        ...
        /** A distinct rate on the rate card. You can choose to use this rate rather than list rate when consuming a credit or commit. */
        fun commitRate(commitRate: CommitRate) =
            apply {
                body.commitRate(commitRate)
            }
        ...
        /** Only set for CUSTOM rate_type. This field is interpreted by custom rate processors. */
        fun customRate(customRate: CustomRate) =
            apply {
                body.customRate(customRate)
            }
        ...
        /** Default proration configuration. Only valid for SUBSCRIPTION rate_type. Must be set to true. */
        fun isProrated(isProrated: Boolean) =
            apply {
                body.isProrated(isProrated)
            }
        ...
        /**
         * Returns an immutable instance of [RateAddParams].
         *
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (entitled.asKnown().isPresent) 1 else 0) + (if (productId.asKnown().isPresent) 1 else 0) + (if (rateCardId.asKnown().isPresent) 1 else 0) + (rateType.asKnown().getOrNull()?.validity() ?: 0) + (if (startingAt.asKnown().isPresent) 1 else 0) + (billingFrequency.asKnown().getOrNull()?.validity() ?: 0) + (commitRate.asKnown().getOrNull()?.validity() ?: 0) + (if (creditTypeId.asKnown().isPresent) 1 else 0) + (customRate.asKnown().getOrNull()?.validity() ?: 0) + (if (endingBefore.asKnown().isPresent) 1 else 0) + (if (isProrated.asKnown().isPresent) 1 else 0) + (if (price.asKnown().isPresent) 1 else 0) + (pricingGroupValues.asKnown().getOrNull()?.validity() ?: 0) + (if (quantity.asKnown().isPresent) 1 else 0) + (tiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Body && entitled == other.entitled && productId == other.productId && rateCardId == other.rateCardId && rateType == other.rateType && startingAt == other.startingAt && billingFrequency == other.billingFrequency && commitRate == other.commitRate && creditTypeId == other.creditTypeId && customRate == other.customRate && endingBefore == other.endingBefore && isProrated == other.isProrated && price == other.price && pricingGroupValues == other.pricingGroupValues && quantity == other.quantity && tiers == other.tiers && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(entitled, productId, rateCardId, rateType, startingAt, billingFrequency, commitRate, creditTypeId, customRate, endingBefore, isProrated, price, pricingGroupValues, quantity, tiers, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Body{entitled=$entitled, productId=$productId, rateCardId=$rateCardId, rateType=$rateType, startingAt=$startingAt, billingFrequency=$billingFrequency, commitRate=$commitRate, creditTypeId=$creditTypeId, customRate=$customRate, endingBefore=$endingBefore, isProrated=$isProrated, price=$price, pricingGroupValues=$pricingGroupValues, quantity=$quantity, tiers=$tiers, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is RateAddParams && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams
    }

    override fun hashCode(): Int = Objects.hash(body, additionalHeaders, additionalQueryParams)

    override fun toString() = "RateAddParams{body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
