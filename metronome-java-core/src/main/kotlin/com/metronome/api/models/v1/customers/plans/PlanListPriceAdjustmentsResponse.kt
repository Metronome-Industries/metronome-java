...
class PlanListPriceAdjustmentsResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [PlanListPriceAdjustmentsResponse].
         *
    ...
    class Builder internal constructor() {
        ...
        internal fun from(planListPriceAdjustmentsResponse: PlanListPriceAdjustmentsResponse) =
            apply {
                ...
                additionalProperties = planListPriceAdjustmentsResponse.additionalProperties.toMutableMap()
            }
     ...
     *   expected type.
     */
    fun validate(): PlanListPriceAdjustmentsResponse =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (chargeId.asKnown().isPresent) 1 else 0) + (chargeType.asKnown().getOrNull()?.validity() ?: 0) + (prices.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (startPeriod.asKnown().isPresent) 1 else 0) + (if (quantity.asKnown().isPresent) 1 else 0)
    ...
    class Price @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (adjustmentType.asKnown().getOrNull()?.validity() ?: 0) + (if (quantity.asKnown().isPresent) 1 else 0) + (if (tier.asKnown().isPresent) 1 else 0) + (if (value.asKnown().isPresent) 1 else 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Price && adjustmentType == other.adjustmentType && quantity == other.quantity && tier == other.tier && value == other.value && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(adjustmentType, quantity, tier, value, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Price{adjustmentType=$adjustmentType, quantity=$quantity, tier=$tier, value=$value, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is PlanListPriceAdjustmentsResponse && chargeId == other.chargeId && chargeType == other.chargeType && prices == other.prices && startPeriod == other.startPeriod && quantity == other.quantity && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "PlanListPriceAdjustmentsResponse{chargeId=$chargeId, chargeType=$chargeType, prices=$prices, startPeriod=$startPeriod, quantity=$quantity, additionalProperties=$additionalProperties}"
}
