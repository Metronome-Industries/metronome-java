...
class Rate @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (rateType.asKnown().getOrNull()?.validity() ?: 0) + (creditType.asKnown().getOrNull()?.validity() ?: 0) + (customRate.asKnown().getOrNull()?.validity() ?: 0) + (if (isProrated.asKnown().isPresent) 1 else 0) + (if (price.asKnown().isPresent) 1 else 0) + (pricingGroupValues.asKnown().getOrNull()?.validity() ?: 0) + (if (quantity.asKnown().isPresent) 1 else 0) + (tiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is Rate && rateType == other.rateType && creditType == other.creditType && customRate == other.customRate && isProrated == other.isProrated && price == other.price && pricingGroupValues == other.pricingGroupValues && quantity == other.quantity && tiers == other.tiers && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "Rate{rateType=$rateType, creditType=$creditType, customRate=$customRate, isProrated=$isProrated, price=$price, pricingGroupValues=$pricingGroupValues, quantity=$quantity, tiers=$tiers, additionalProperties=$additionalProperties}"
}
