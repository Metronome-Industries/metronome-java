...
class CreditLedgerEntry @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    private constructor(
        ...
        @JsonProperty("running_balance") @ExcludeMissing runningBalance: JsonField<Double> = JsonMissing.of(),
    ...
    class Builder internal constructor() {
         ...
         *
         * You should usually call [Builder.runningBalance] with a well-typed [Double] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun runningBalance(runningBalance: JsonField<Double>) =
            apply {
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (amount.asKnown().isPresent) 1 else 0) + (if (createdBy.asKnown().isPresent) 1 else 0) + (if (creditGrantId.asKnown().isPresent) 1 else 0) + (if (effectiveAt.asKnown().isPresent) 1 else 0) + (if (reason.asKnown().isPresent) 1 else 0) + (if (runningBalance.asKnown().isPresent) 1 else 0) + (if (invoiceId.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is CreditLedgerEntry && amount == other.amount && createdBy == other.createdBy && creditGrantId == other.creditGrantId && effectiveAt == other.effectiveAt && reason == other.reason && runningBalance == other.runningBalance && invoiceId == other.invoiceId && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "CreditLedgerEntry{amount=$amount, createdBy=$createdBy, creditGrantId=$creditGrantId, effectiveAt=$effectiveAt, reason=$reason, runningBalance=$runningBalance, invoiceId=$invoiceId, additionalProperties=$additionalProperties}"
}
