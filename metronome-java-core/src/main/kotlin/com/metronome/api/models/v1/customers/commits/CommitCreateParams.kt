 ...
 *
 */
class CommitCreateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(commitCreateParams: CommitCreateParams) =
            apply {
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (accessSchedule.asKnown().getOrNull()?.validity() ?: 0) + (if (customerId.asKnown().isPresent) 1 else 0) + (if (priority.asKnown().isPresent) 1 else 0) + (if (productId.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (applicableContractIds.asKnown().getOrNull()?.size ?: 0) + (applicableProductIds.asKnown().getOrNull()?.size ?: 0) + (applicableProductTags.asKnown().getOrNull()?.size ?: 0) + (customFields.asKnown().getOrNull()?.validity() ?: 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (invoiceContractId.asKnown().isPresent) 1 else 0) + (invoiceSchedule.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (netsuiteSalesOrderId.asKnown().isPresent) 1 else 0) + (rateType.asKnown().getOrNull()?.validity() ?: 0) + (if (salesforceOpportunityId.asKnown().isPresent) 1 else 0) + (specifiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (uniquenessKey.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is CommitCreateParams && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams
    }

    override fun hashCode(): Int = Objects.hash(body, additionalHeaders, additionalQueryParams)

    override fun toString() = "CommitCreateParams{body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
