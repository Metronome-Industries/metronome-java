 ...
 *
 */
class ContractGetNetBalanceParams private constructor(
     ...
     *
     * Unlike [invoiceInclusionMode], this method doesn't throw if the JSON field has an unexpected type.
     */
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(contractGetNetBalanceParams: ContractGetNetBalanceParams) =
            apply {
        ...
        /** Balance filters are OR'd together, so if a given commit or credit matches any of the filters, it will be included in the net balance. */
        fun filters(filters: List<BalanceFilter>) =
            apply {
         ...
         *
         * You should usually call [Builder.filters] with a well-typed `List<BalanceFilter>` value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun filters(filters: JsonField<List<BalanceFilter>>) =
            apply {
                body.filters(filters)
            }
         ...
         */
        fun addFilter(filter: BalanceFilter) =
            apply {
         ...
         *
         * You should usually call [Builder.invoiceInclusionMode] with a well-typed [InvoiceInclusionMode] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
        ...
        private constructor(
            ...
            @JsonProperty("filters") @ExcludeMissing filters: JsonField<List<BalanceFilter>> = JsonMissing.of(),
            @JsonProperty("invoice_inclusion_mode") @ExcludeMissing invoiceInclusionMode: JsonField<InvoiceInclusionMode> = JsonMissing.of()
        ) : this(
          customerId,
          creditTypeId,
          filters,
         ...
         * @throws MetronomeInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun invoiceInclusionMode(): Optional<InvoiceInclusionMode> = invoiceInclusionMode.getOptional("invoice_inclusion_mode")
        ...
        class Builder internal constructor() {
            ...
            fun addFilter(filter: BalanceFilter) =
                apply {
                    filters = (filters ?: JsonField.of(mutableListOf())).also {
                        checkKnown("filters", it).add(filter)
                    }
                }

            /** Controls which invoices are considered when calculating the remaining balance. `FINALIZED` considers only deductions from finalized invoices. `FINALIZED_AND_DRAFT` also includes deductions from pending draft invoices. */
            fun invoiceInclusionMode(invoiceInclusionMode: InvoiceInclusionMode) = invoiceInclusionMode(JsonField.of(invoiceInclusionMode))
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (customerId.asKnown().isPresent) 1 else 0) + (if (creditTypeId.asKnown().isPresent) 1 else 0) + (filters.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (invoiceInclusionMode.asKnown().getOrNull()?.validity() ?: 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Body && customerId == other.customerId && creditTypeId == other.creditTypeId && filters == other.filters && invoiceInclusionMode == other.invoiceInclusionMode && additionalProperties == other.additionalProperties
        }
        ...
        override fun toString() = "Body{customerId=$customerId, creditTypeId=$creditTypeId, filters=$filters, invoiceInclusionMode=$invoiceInclusionMode, additionalProperties=$additionalProperties}"
    }

    /** Controls which invoices are considered when calculating the remaining balance. `FINALIZED` considers only deductions from finalized invoices. `FINALIZED_AND_DRAFT` also includes deductions from pending draft invoices. */
    class InvoiceInclusionMode @JsonCreator private constructor(
        ...
        /**
         * An enum containing [InvoiceInclusionMode]'s known values, as well as an [_UNKNOWN] member.
         *
        ...
        enum class Value {
            FINALIZED,
            FINALIZED_AND_DRAFT,
            /** An enum member indicating that [InvoiceInclusionMode] was instantiated with an unknown value. */
            _UNKNOWN,
        }
         ...
         *   expected type.
         */
        fun validate(): InvoiceInclusionMode =
            apply {
                if (validated) {
                  return@apply
                }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is ContractGetNetBalanceParams && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams
    }

    override fun hashCode(): Int = Objects.hash(body, additionalHeaders, additionalQueryParams)

    override fun toString() = "ContractGetNetBalanceParams{body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
