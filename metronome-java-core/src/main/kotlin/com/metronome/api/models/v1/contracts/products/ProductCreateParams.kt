...
class ProductCreateParams private constructor(
     ...
     * @throws MetronomeInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun sqlBreakdownGranularity(): Optional<SqlBreakdownGranularity> = body.sqlBreakdownGranularity()
     ...
     *
     * Unlike [sqlBreakdownGranularity], this method doesn't throw if the JSON field has an unexpected type.
     */
    fun _sqlBreakdownGranularity(): JsonField<SqlBreakdownGranularity> = body._sqlBreakdownGranularity()
    ...
    class Builder internal constructor() {
        ...
        fun addCompositeTag(compositeTag: String) =
            apply {
                body.addCompositeTag(compositeTag)
            }
        ...
        /** This field's availability is dependent on your client's configuration. Defaults to true. */
        fun isRefundable(isRefundable: Boolean) =
            apply {
         ...
         *
         * You should usually call [Builder.sqlBreakdownGranularity] with a well-typed [SqlBreakdownGranularity] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
        ...
        private constructor(
            ...
            @JsonProperty("sql_breakdown_granularity") @ExcludeMissing sqlBreakdownGranularity: JsonField<SqlBreakdownGranularity> = JsonMissing.of(),
         ...
         * @throws MetronomeInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun sqlBreakdownGranularity(): Optional<SqlBreakdownGranularity> = sqlBreakdownGranularity.getOptional("sql_breakdown_granularity")
        ...
        class Builder internal constructor() {
            ...
            private var sqlBreakdownGranularity: JsonField<SqlBreakdownGranularity> = JsonMissing.of()
            ...
            /** Defines the breakdown behavior when calculating usage from SQL Billable Metrics. If set to 'service_period' (default), the usage will be evaluated once for all events the invoice service period and the usage will be applied at the last instant of the invoice. If set to 'hour', it will be broken down and evaluated for each hour. For most use cases, 'hour' is recommended. The setting has no effect for Streaming Billable Metrics. */
            fun sqlBreakdownGranularity(sqlBreakdownGranularity: SqlBreakdownGranularity) = sqlBreakdownGranularity(JsonField.of(sqlBreakdownGranularity))
            ...
            fun sqlBreakdownGranularity(sqlBreakdownGranularity: JsonField<SqlBreakdownGranularity>) =
                apply {
                    this.sqlBreakdownGranularity = sqlBreakdownGranularity
                }
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (name.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (billableMetricId.asKnown().isPresent) 1 else 0) + (compositeProductIds.asKnown().getOrNull()?.size ?: 0) + (compositeTags.asKnown().getOrNull()?.size ?: 0) + (customFields.asKnown().getOrNull()?.validity() ?: 0) + (if (excludeFreeUsage.asKnown().isPresent) 1 else 0) + (if (isRefundable.asKnown().isPresent) 1 else 0) + (if (netsuiteInternalItemId.asKnown().isPresent) 1 else 0) + (if (netsuiteOverageItemId.asKnown().isPresent) 1 else 0) + (presentationGroupKey.asKnown().getOrNull()?.size ?: 0) + (pricingGroupKey.asKnown().getOrNull()?.size ?: 0) + (quantityConversion.asKnown().getOrNull()?.validity() ?: 0) + (quantityRounding.asKnown().getOrNull()?.validity() ?: 0) + (sqlBreakdownGranularity.asKnown().getOrNull()?.validity() ?: 0) + (tags.asKnown().getOrNull()?.size ?: 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Body && name == other.name && type == other.type && billableMetricId == other.billableMetricId && compositeProductIds == other.compositeProductIds && compositeTags == other.compositeTags && customFields == other.customFields && excludeFreeUsage == other.excludeFreeUsage && isRefundable == other.isRefundable && netsuiteInternalItemId == other.netsuiteInternalItemId && netsuiteOverageItemId == other.netsuiteOverageItemId && presentationGroupKey == other.presentationGroupKey && pricingGroupKey == other.pricingGroupKey && quantityConversion == other.quantityConversion && quantityRounding == other.quantityRounding && sqlBreakdownGranularity == other.sqlBreakdownGranularity && tags == other.tags && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(name, type, billableMetricId, compositeProductIds, compositeTags, customFields, excludeFreeUsage, isRefundable, netsuiteInternalItemId, netsuiteOverageItemId, presentationGroupKey, pricingGroupKey, quantityConversion, quantityRounding, sqlBreakdownGranularity, tags, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Body{name=$name, type=$type, billableMetricId=$billableMetricId, compositeProductIds=$compositeProductIds, compositeTags=$compositeTags, customFields=$customFields, excludeFreeUsage=$excludeFreeUsage, isRefundable=$isRefundable, netsuiteInternalItemId=$netsuiteInternalItemId, netsuiteOverageItemId=$netsuiteOverageItemId, presentationGroupKey=$presentationGroupKey, pricingGroupKey=$pricingGroupKey, quantityConversion=$quantityConversion, quantityRounding=$quantityRounding, sqlBreakdownGranularity=$sqlBreakdownGranularity, tags=$tags, additionalProperties=$additionalProperties}"
    }
    ...
    /** Defines the breakdown behavior when calculating usage from SQL Billable Metrics. If set to 'service_period' (default), the usage will be evaluated once for all events the invoice service period and the usage will be applied at the last instant of the invoice. If set to 'hour', it will be broken down and evaluated for each hour. For most use cases, 'hour' is recommended. The setting has no effect for Streaming Billable Metrics. */
    class SqlBreakdownGranularity @JsonCreator private constructor(
        private val value: JsonField<String>,
        ...
        /**
         * An enum containing [SqlBreakdownGranularity]'s known values, as well as an [_UNKNOWN] member.
         *
         * An instance of [SqlBreakdownGranularity] can contain an unknown value in a couple of cases:
         *
         * - It was deserialized from data that doesn't match any known member. For example, if the SDK is on
         *   an older version than the API, then the API may respond with new members that the SDK is unaware
         *   of.
        ...
        enum class Value {
            HOUR,
            SERVICE_PERIOD,
            /** An enum member indicating that [SqlBreakdownGranularity] was instantiated with an unknown value. */
            _UNKNOWN,
        }
        ...
        fun known(): Known =
            when (this) {
                ...
                else -> throw MetronomeInvalidDataException("Unknown SqlBreakdownGranularity: $value")
            }
         ...
         *   expected type.
         */
        fun validate(): SqlBreakdownGranularity =
            apply {
                if (validated) {
                  return@apply
                }
