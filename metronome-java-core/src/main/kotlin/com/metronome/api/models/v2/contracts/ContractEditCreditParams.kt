 ...
 *
 */
class ContractEditCreditParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(contractEditCreditParams: ContractEditCreditParams) =
            apply {
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (creditId.asKnown().isPresent) 1 else 0) + (if (customerId.asKnown().isPresent) 1 else 0) + (accessSchedule.asKnown().getOrNull()?.validity() ?: 0) + (applicableContractIds.asKnown().getOrNull()?.size ?: 0) + (applicableProductIds.asKnown().getOrNull()?.size ?: 0) + (applicableProductTags.asKnown().getOrNull()?.size ?: 0) + (if (description.asKnown().isPresent) 1 else 0) + (hierarchyConfiguration.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (priority.asKnown().isPresent) 1 else 0) + (if (productId.asKnown().isPresent) 1 else 0) + (rateType.asKnown().getOrNull()?.validity() ?: 0) + (specifiers.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
        ...
        override fun equals(other: Any?): Boolean {
          ...
          return other is Body && creditId == other.creditId && customerId == other.customerId && accessSchedule == other.accessSchedule && applicableContractIds == other.applicableContractIds && applicableProductIds == other.applicableProductIds && applicableProductTags == other.applicableProductTags && description == other.description && hierarchyConfiguration == other.hierarchyConfiguration && name == other.name && priority == other.priority && productId == other.productId && rateType == other.rateType && specifiers == other.specifiers && additionalProperties == other.additionalProperties
        }

        private val hashCode: Int by lazy { Objects.hash(creditId, customerId, accessSchedule, applicableContractIds, applicableProductIds, applicableProductTags, description, hierarchyConfiguration, name, priority, productId, rateType, specifiers, additionalProperties) }

        override fun hashCode(): Int = hashCode

        override fun toString() = "Body{creditId=$creditId, customerId=$customerId, accessSchedule=$accessSchedule, applicableContractIds=$applicableContractIds, applicableProductIds=$applicableProductIds, applicableProductTags=$applicableProductTags, description=$description, hierarchyConfiguration=$hierarchyConfiguration, name=$name, priority=$priority, productId=$productId, rateType=$rateType, specifiers=$specifiers, additionalProperties=$additionalProperties}"
    }
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is ContractEditCreditParams && body == other.body && additionalHeaders == other.additionalHeaders && additionalQueryParams == other.additionalQueryParams
    }

    override fun hashCode(): Int = Objects.hash(body, additionalHeaders, additionalQueryParams)

    override fun toString() = "ContractEditCreditParams{body=$body, additionalHeaders=$additionalHeaders, additionalQueryParams=$additionalQueryParams}"
}
