...
class PlanListResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (customFields.asKnown().getOrNull()?.validity() ?: 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is PlanListResponse && id == other.id && description == other.description && name == other.name && customFields == other.customFields && additionalProperties == other.additionalProperties
    }
    ...
    override fun toString() = "PlanListResponse{id=$id, description=$description, name=$name, customFields=$customFields, additionalProperties=$additionalProperties}"
}
