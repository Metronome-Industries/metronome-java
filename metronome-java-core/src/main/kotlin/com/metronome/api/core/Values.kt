...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        when (this) {
            is JsonString -> value
            else -> throw MetronomeInvalidDataException("Value is not a string")
        }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw MetronomeInvalidDataException("'${name}' is not set")
            is JsonNull -> throw MetronomeInvalidDataException("'${name}' is null")
            else -> throw MetronomeInvalidDataException("'${name}' is invalid, received ${this}")
        }
