...
class UsageListWithGroupsResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    @JsonProperty("group")
    @ExcludeMissing
    fun _group(): JsonField<Group> = group
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(usageListWithGroupsResponse: UsageListWithGroupsResponse) =
            apply {
         ...
         *
         * You should usually call [Builder.group] with a well-typed [Group] value instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun group(group: JsonField<Group>) =
            apply {
                this.group = group
            }
     ...
     *   expected type.
     */
    fun validate(): UsageListWithGroupsResponse =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (endingBefore.asKnown().isPresent) 1 else 0) + (if (groupKey.asKnown().isPresent) 1 else 0) + (if (groupValue.asKnown().isPresent) 1 else 0) + (if (startingOn.asKnown().isPresent) 1 else 0) + (if (value.asKnown().isPresent) 1 else 0) + (group.asKnown().getOrNull()?.validity() ?: 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is UsageListWithGroupsResponse && endingBefore == other.endingBefore && groupKey == other.groupKey && groupValue == other.groupValue && startingOn == other.startingOn && value == other.value && group == other.group && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(endingBefore, groupKey, groupValue, startingOn, value, group, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "UsageListWithGroupsResponse{endingBefore=$endingBefore, groupKey=$groupKey, groupValue=$groupValue, startingOn=$startingOn, value=$value, group=$group, additionalProperties=$additionalProperties}"
}
