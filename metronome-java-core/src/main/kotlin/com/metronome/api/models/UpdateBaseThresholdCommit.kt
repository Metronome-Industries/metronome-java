...
class UpdateBaseThresholdCommit @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    /**
     * The priority of the commit, used to determine drawdown order. Lower priority commits are consumed first. Defaults to 100 if not specified. On updates, set to null to clear a previously configured priority.
     *
     * @throws MetronomeInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(updateBaseThresholdCommit: UpdateBaseThresholdCommit) =
            apply {
     ...
     *   expected type.
     */
    fun validate(): UpdateBaseThresholdCommit =
        apply {
            if (validated) {
              return@apply
            }
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (description.asKnown().isPresent) 1 else 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (priority.asKnown().isPresent) 1 else 0) + (if (productId.asKnown().isPresent) 1 else 0)
    ...
    override fun equals(other: Any?): Boolean {
      ...
      return other is UpdateBaseThresholdCommit && description == other.description && name == other.name && priority == other.priority && productId == other.productId && additionalProperties == other.additionalProperties
    }

    private val hashCode: Int by lazy { Objects.hash(description, name, priority, productId, additionalProperties) }

    override fun hashCode(): Int = hashCode

    override fun toString() = "UpdateBaseThresholdCommit{description=$description, name=$name, priority=$priority, productId=$productId, additionalProperties=$additionalProperties}"
}
